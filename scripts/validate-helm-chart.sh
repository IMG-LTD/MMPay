...
require_pattern templates/configmap.yaml 'SPRING_DATASOURCE_URL' "ConfigMap must expose Spring Boot datasource URL"
require_pattern templates/configmap.yaml 'SPRING_DATASOURCE_USERNAME' "ConfigMap must expose Spring Boot datasource username"
require_pattern templates/deployment.yaml 'SPRING_DATASOURCE_PASSWORD' "Deployment must expose Spring Boot datasource password"
require_pattern templates/deployment.yaml 'MMPAY_AUDIT_HMAC_KEY' "Deployment must expose audit HMAC key through env from secret"
require_pattern values.yaml 'auditHmacKey: "audit-hmac-key"' "Helm values must define the external audit HMAC key name"
require_pattern templates/deployment.yaml 'HUIFU_RSA_PRIVATE_KEY' "Deployment must expose Huifu RSA private key through env from secret"
require_pattern templates/deployment.yaml 'HUIFU_WEBHOOK_ENDPOINT_KEY' "Deployment must expose Huifu webhook endpoint key through env from secret"
...
reject_pattern templates/configmap.yaml 'MMPAY_DATASOURCE_URL' "ConfigMap must not use non-Spring datasource URL env names"
reject_pattern templates/deployment.yaml 'MMPAY_DATASOURCE_PASSWORD' "Deployment must not use non-Spring datasource password env names"
reject_pattern templates/deployment.yaml 'value: .*private-key' "Deployment must not inline provider private key values"
